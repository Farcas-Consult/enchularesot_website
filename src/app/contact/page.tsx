...
const Contact: React.FC = () => {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // Remove the error for the current field
    const rest = Object.fromEntries(Object.entries(errors).filter(([k]) => k !== e.target.name));
    setErrors(rest);
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-10 mb-12">
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-[#DCC7A1] shadow p-8 flex flex-col gap-5">
            ...
            {success && (
              <div className="p-2 rounded bg-(--brand-background) border border-(--brand-primary)/30 text-(--brand-primary) text-center text-xs flex items-center justify-center gap-2 font-nunito">
        ...
        {/* Google Map Embed */}
              Get Directions
          {/* Note: 'loading' attribute on iframe is not supported by Safari on iOS < 16.4. */}
          <iframe
